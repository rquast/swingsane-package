...
/**
 * A base descriptor class used to reference a Component panel for the Wizard, as well as provide
...
public class WizardPanelDescriptor {
  ...
  /**
   * Identifier returned by getNextPanelDescriptor() to indicate that this is the last panel and the
   * text of the 'Next' button should change to 'Finish'.
   */
  ...
  public WizardPanelDescriptor() {
    panelIdentifier = WizardPanelDescriptor.DEFAULT_PANEL_IDENTIFIER;
    targetPanel = new JPanel();
  }

  /**
   * Constructor which accepts both the Object-based identifier and a reference to the Component
   * class which makes up the panel.
   * 
   * @param id
   *          Object-based identifier
   * @param panel
   *          A class which extends java.awt.Component that will be inserted as a panel into the
   *          wizard dialog.
   */
  ...
  /**
   * Sets the Object-based identifier for this panel. The identifier must be unique from all the
   * other identifiers in the panel.
   * 
   * @param id
  ...
  /**
   * Override this class to provide the Object-based identifier of the panel that the user should
   * traverse to when the Next button is pressed. Note that this method is only called when the
   * button is actually pressed, so that the panel can change the next panel's identifier
   * dynamically at runtime if necessary. Return null if the button should be disabled. Return
   * FinishIdentfier if the button text should change to 'Finish' and the dialog should end.
   * 
  ...
  /**
   * Override this class to provide the Object-based identifier of the panel that the user should
   * traverse to when the Back button is pressed. Note that this method is only called when the
   * button is actually pressed, so that the panel can change the previous panel's identifier
   * dynamically at runtime if necessary. Return null if the button should be disabled.
   * 
   * @return Object-based identifier
   */
  ...
  /**
   * Override this method to provide functionality that will be performed just before the panel is
   * to be displayed.
   */
