...
/**
 * Helper class to avoid running more than one concurrent instance of a Java program.
 * 
 * <h3>Rationale</h3> With some applications it may be useless or even dangerous to run more than
 * one instance at the same time. This class helps a Java application to determine if another
 * instance is running already. It does so by trying to reserve a server socket for a given port. If
 * the server socket can be reserved, this instance is the first one. Otherwise a user-defined byte
 * signature is sent to the running instance, which can then perform some action (e.g. bringing its
 * own window to the front of the desktop).
 * 
 * <h3>Usage</h3> Study the MultipleInstanceTest application that comes with this class in order to
 * find out how to integrate the check in your own application.
 * 
...
public class MultipleInstanceChecker {
   ...
   *          a byte array of length 8 or larger
   * @param port
   *          number of the port for which a server socket is to be reserved
   */
  public MultipleInstanceChecker(byte[] applicationSignature, int port) {
    if (applicationSignature == null || applicationSignature.length != SIGNATURE_LENGTH) {
      throw new IllegalArgumentException("Signature length in bytes must be " + SIGNATURE_LENGTH);
    }
