...
import com.swingsane.installer.Main;

/**
 * This class implements a basic wizard dialog, where the programmer can insert one or more
 * Components to act as panels. These panels can be navigated through arbitrarily using the 'Next'
 * or 'Back' buttons, or the dialog itself can be closed using the 'Cancel' button. Note that even
 * though the dialog uses a CardLayout manager, the order of the panels is not linear. Each panel
...
public class Wizard extends WindowAdapter implements PropertyChangeListener {
   ...
   * 
   * @param owner
   *          The java.awt.Frame object that is the owner of the javax.swing.JDialog.
   * @param main
   */
  public Wizard(Frame owner, Main main) {
    this.main = main;
    wizardModel = new WizardModel();
    wizardDialog = new JDialog(owner);
    initComponents();
  ...
  /**
   * Returns an instance of the JDialog that this class created. This is useful in the event that
   * you want to change any of the JDialog parameters manually.
   * 
   ...
   * 
   * @return The owner (java.awt.Frame or java.awt.Dialog) of the javax.swing.JDialog generated by
   *         this class.
   */
  ...
  /**
   * Convienence method that displays a modal wizard dialog and blocks until the dialog has
   * completed.
   * 
  ...
  /**
   * Add a Component as a panel for the wizard dialog by registering its WizardPanelDescriptor
   * object. Each panel is identified by a unique Object-based identifier (often a String), which
   * can be used by the setCurrentPanel() method to display the panel at runtime.
   * 
   * @param id
   *          An Object-based identifier used to identify the WizardPanelDescriptor object.
   * @param panel
   *          The WizardPanelDescriptor object which contains helpful information about the panel.
   */
  ...
  /**
   * Displays the panel identified by the object passed in. This is the same Object-based identified
   * used when registering the panel.
   * 
   * @param id
  ...
  public void setCurrentPanel(Object id) {
    ...
    if (oldPanelDescriptor != null) {
      nextPanelOK = oldPanelDescriptor.aboutToHidePanel();
    }

    if (nextPanelOK == false) {
      return;
    }
  ...
  /**
   * Method used to listen for property change events from the model and update the dialog's
   * graphical components as necessary.
   * 
   * @param evt
   *          PropertyChangeEvent passed from the model to signal that one of its properties has
   *          changed value.
   */
  ...
  void close(int code) {
    ...
    if (code == 0) {
      ...
      if (mainInstance.launchCheckBox.isSelected() == true) {
        if (Main.WINDOWS) {
          try {
            String[] cmdArray = { mainInstance.outputDir.getPath() + "/swingsane.exe" };
          ...
          try {
            String[] cmdArray = { "open", mainInstance.outputDir.getPath() + "/SwingSane.app" };
            Runtime.getRuntime().exec(cmdArray);
          } catch (Exception ex) {
            System.out.println(ex.getMessage());
            ex.printStackTrace();
          ...
          try {
            String[] cmdArray = { mainInstance.outputDir.getPath() + "/swingsane.sh" };
            Runtime.getRuntime().exec(cmdArray);
          } catch (Exception ex) {
            System.out.println(ex.getMessage());
            ex.printStackTrace();
  ...
  /**
   * This method initializes the components for the wizard dialog: it creates a JDialog as a
   * CardLayout panel surrounded by a small amount of space on each side, as well as three buttons
   * at the bottom.
   */
  ...
  private void initComponents() {
    ...
    backButton.setActionCommand(Wizard.BACK_BUTTON_ACTION_COMMAND);
    nextButton.setActionCommand(Wizard.NEXT_BUTTON_ACTION_COMMAND);
    cancelButton.setActionCommand(Wizard.CANCEL_BUTTON_ACTION_COMMAND);
    ...
    buttonBox.setBorder(new EmptyBorder(new Insets(5, 10, 5, 20)));
  ...
  public void windowClosing(WindowEvent e) {
    returnCode = Wizard.CANCEL_RETURN_CODE;
  }
